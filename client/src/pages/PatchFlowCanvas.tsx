...
import { buildPatchFlowConnections, buildPatchFlowGroups, filterPatchFlowEntities, getActivePatchFlowEntities, getPatchFlowSourceOptions, limitPatchFlowEntities, type PatchEntityType, type PatchFlowConnection, type PatchFlowEntity, type PatchFlowGroup, type PatchFlowLayout } from "@/lib/patchFlowGraph";
...
import { Focus, Maximize2, Search, SlidersHorizontal, Sparkles, X } from "lucide-react";
  ...
  return <div className={`min-w-[320px] rounded-xl border p-3 shadow-sm backdrop-blur ${tokens.shell}`}>
    ...
    <div className="grid max-h-44 grid-cols-4 gap-1.5 overflow-y-auto pr-1">
      {data.entities.map((entity, index) => {
        ...
        const dimmed = (data.focusEnabled || data.emphasizeActiveRoute) && !traced;
...
export default function PatchFlowCanvas({ entities, selectedNodeId, traceRoles, focusedNodeIds, focusEnabled, onSelectNode, onToggleFocus, onClearFocus, preferenceKey = "default" }: PatchFlowCanvasProps) {
  const initialPreferences = readCanvasPreference(preferenceKey);
  const [query, setQuery] = useState("");
  const [sourceGroup, setSourceGroup] = useState("all");
  const [entityType, setEntityType] = useState<"all" | PatchEntityType>("all");
  const [showAllPatches, setShowAllPatches] = useState(initialPreferences.showAllPatches);
  const [layout, setLayout] = useState<PatchFlowLayout>(initialPreferences.layout);
  const [emphasizeActiveRoute, setEmphasizeActiveRoute] = useState(initialPreferences.emphasizeActiveRoute);
  const [selectedCable, setSelectedCable] = useState<PatchFlowConnection | null>(null);
  ...
  const groups = useMemo(() => buildPatchFlowGroups(filteredEntities, layout), [filteredEntities, layout]);
  ...
  const connections = useMemo(() => buildPatchFlowConnections(entityMap, groups, visibleEntityIds, traceRoles, focusEnabled), [entityMap, focusEnabled, groups, traceRoles, visibleEntityIds]);
  const selectedConnection = useMemo(() => selectedCable && connections.find((connection) => connection.source === selectedCable.source && connection.target === selectedCable.target) ? selectedCable : null, [connections, selectedCable]);
  const visibleRouteKey = useMemo(() => `${layout}:${filteredEntities.map((entity) => entity.id).join("|")}`, [filteredEntities, layout]);
  const activeCableCount = connections.filter((connection) => connection.active).length;
  const selectedCableSource = selectedConnection ? entityMap.get(selectedConnection.source) : null;
  const selectedCableTarget = selectedConnection ? entityMap.get(selectedConnection.target) : null;

  useEffect(() => {
    const preferences = readCanvasPreference(preferenceKey);
    setLayout(preferences.layout);
    setShowAllPatches(preferences.showAllPatches);
    setEmphasizeActiveRoute(preferences.emphasizeActiveRoute);
    setSelectedCable(null);
  }, [preferenceKey]);

  useEffect(() => {
    try {
      window.localStorage.setItem(`wingtools.patch-flow.${preferenceKey}`, JSON.stringify({ layout, showAllPatches, emphasizeActiveRoute }));
    } catch {
      // Local storage can be unavailable in private or restricted browser sessions.
    }
  }, [emphasizeActiveRoute, layout, preferenceKey, showAllPatches]);

  useEffect(() => {
    const timer = window.setTimeout(() => flowRef.current?.fitView({ padding: 0.12, duration: 280 }), 0);
  ...
  const nodes = useMemo<Node[]>(() => groups.map((group) => ({
    ...
    data: { ...group, selectedNodeId, traceRoles, focusEnabled, emphasizeActiveRoute, onSelectEntity: onSelectNode },
    draggable: false,
    selectable: false,
    style: { width: group.entities.length > 24 ? 660 : group.entities.length > 12 ? 480 : 360 },
  })), [emphasizeActiveRoute, focusEnabled, groups, onSelectNode, selectedNodeId, traceRoles]);
  const edges = useMemo<Edge[]>(() => connections.map((connection, index) => {
    const selected = selectedConnection?.source === connection.source && selectedConnection?.target === connection.target;
    const muted = emphasizeActiveRoute && !connection.active;
    return {
      ...
      type: "smoothstep",
      animated: connection.active || selected,
      data: connection as unknown as Record<string, unknown>,
      style: { stroke: selected ? "#f59e0b" : connection.active ? connection.color : "#94a3b8", strokeWidth: selected ? 3.8 : connection.active ? 2.5 : 1.1, opacity: muted ? 0.12 : connection.active ? 0.95 : 0.46 },
      markerEnd: connection.active ? { type: MarkerType.ArrowClosed, color: connection.color, width: 14, height: 14 } : undefined,
    };
  }), [connections, emphasizeActiveRoute, selectedConnection]);
  const hasFilters = Boolean(query || sourceGroup !== "all" || entityType !== "all");

  return <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-950" data-testid="patch-flow-canvas" data-layout={layout}>
    <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-slate-50 px-4 py-3 dark:border-slate-800 dark:bg-slate-900 sm:px-5"><div><p className="text-xs font-bold uppercase tracking-[0.15em] text-slate-500">Patch Flow Diagram</p><p className="mt-1 text-sm text-slate-600 dark:text-slate-300">Grouped patch points with visible source-to-destination cable paths.</p></div><div className="flex flex-wrap items-center gap-2"><span className="rounded-full border border-slate-200 bg-white px-2.5 py-1 text-xs font-semibold text-slate-600 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-300">{filteredEntities.length} visible patches</span><span className="rounded-full border border-indigo-100 bg-indigo-50 px-2.5 py-1 text-xs font-semibold text-indigo-700 dark:border-indigo-900 dark:bg-indigo-950/40 dark:text-indigo-200">{selectedNodeId ? `${activeCableCount} active route cables` : `${connections.length} visible cables`}</span><Button size="sm" variant={showAllPatches ? "default" : "outline"} onClick={() => setShowAllPatches((current) => !current)}>{showAllPatches ? "Showing all patches" : "Show all patches"}</Button>{selectedNodeId && <Button size="sm" variant={focusEnabled ? "default" : "outline"} onClick={onToggleFocus}><Focus className="mr-1.5 h-3.5 w-3.5" />{focusEnabled ? "Route focus on" : "Focus selected route"}</Button>}{selectedNodeId && <Button size="sm" variant="ghost" onClick={onClearFocus}><X className="mr-1.5 h-3.5 w-3.5" />Clear</Button>}</div></div>
    <div className="flex flex-wrap items-center gap-2 border-b border-slate-200 bg-white px-4 py-2.5 text-xs dark:border-slate-800 dark:bg-slate-950 sm:px-5"><span className="font-semibold text-slate-600 dark:text-slate-300">Dynamic view</span><Button size="sm" variant={layout === "lanes" ? "default" : "outline"} onClick={() => setLayout("lanes")}>Routing lanes</Button><Button size="sm" variant={layout === "compact" ? "default" : "outline"} onClick={() => setLayout("compact")}>Compact topology</Button><Button size="sm" variant={emphasizeActiveRoute ? "default" : "outline"} disabled={!selectedNodeId} onClick={() => setEmphasizeActiveRoute((current) => !current)}><Sparkles className="mr-1.5 h-3.5 w-3.5" />{emphasizeActiveRoute ? "Active route emphasized" : "Emphasize active route"}</Button><Button size="sm" variant="ghost" onClick={() => flowRef.current?.fitView({ padding: 0.12, duration: 280 })}><Maximize2 className="mr-1.5 h-3.5 w-3.5" />Reflow canvas</Button>{selectedConnection && <span className="ml-auto max-w-full truncate rounded-md bg-amber-50 px-2.5 py-1 font-semibold text-amber-800 dark:bg-amber-950/40 dark:text-amber-200">Cable selected · {selectedCableSource?.name} → {selectedCableTarget?.name}</span>}</div>
    ...
    <div className="h-[620px] bg-slate-50 dark:bg-slate-950"><ReactFlow nodes={nodes} edges={edges} nodeTypes={nodeTypes} fitView fitViewOptions={{ padding: 0.12 }} minZoom={0.25} maxZoom={1.75} nodesDraggable={false} nodesConnectable={false} elementsSelectable={false} proOptions={{ hideAttribution: true }} onInit={(instance) => { flowRef.current = instance; }} onEdgeClick={(_, edge) => { if (edge.data) setSelectedCable(edge.data as unknown as PatchFlowConnection); }} onPaneClick={() => setSelectedCable(null)}><Background variant={BackgroundVariant.Dots} gap={18} size={1} color="#cbd5e1" /><Controls showInteractive={false} position="bottom-left" /><MiniMap position="bottom-right" nodeColor={(node) => groupTokens[(node.data as GroupNodeData).lane]?.accent || "#94a3b8"} maskColor="rgba(248, 250, 252, 0.72)" /></ReactFlow></div>
  </section>;
}

function readCanvasPreference(preferenceKey: string): { layout: PatchFlowLayout; showAllPatches: boolean; emphasizeActiveRoute: boolean } {
  if (typeof window === "undefined") return { layout: "lanes", showAllPatches: false, emphasizeActiveRoute: false };
  try {
    const stored = JSON.parse(window.localStorage.getItem(`wingtools.patch-flow.${preferenceKey}`) || "{}");
    return { layout: stored.layout === "compact" ? "compact" : "lanes", showAllPatches: Boolean(stored.showAllPatches), emphasizeActiveRoute: Boolean(stored.emphasizeActiveRoute) };
  } catch {
    return { layout: "lanes", showAllPatches: false, emphasizeActiveRoute: false };
  }
}
